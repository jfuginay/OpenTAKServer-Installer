...
if [[ "$BLEEDING_EDGE" -eq 1 ]]; then
  GIT_URL=git+https://github.com/jfuginay/OpenTAKServer.git
...
EOF
  ...
  ~/.opentakserver_venv/bin/lastversion --pre --assets extract jfuginay/OpenTAKServer-UI
...
EOF
  ...
  rm -fr /var/www/html/opentakserver/*
  cd /var/www/html/opentakserver/
  ~/.opentakserver_venv/bin/lastversion --assets extract jfuginay/OpenTAKServer-UI
...
if [[ $? -ne 0 ]]; then
  ...
  sudo wget https://raw.githubusercontent.com/jfuginay/OpenTAKServer-Installer/master/rabbitmq.conf -O /etc/rabbitmq/rabbitmq.conf
